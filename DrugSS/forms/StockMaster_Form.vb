Public Class StockMaster_Form
    ...
    Private Sub frm_StockMaster_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        sql = "SELECT * FROM tblitems"
        reloadDtg(sql, dtglist)
        '------------------------------------------
        dtgcolor(dtglist, 5)
        '------------------------------
        fillcbo("SELECT * FROM `tblsettings` WHERE `UNT`='CATEGORY'", cbotype)
        fillcbo("SELECT * FROM `tblsettings` WHERE `UNT`='Unit'", cbounit)
        '-------------------------
        cleartext(pnl_stockmaster)
        '--------------------------------------
        maxcolumn = dtglist.Columns.Count - 1
    ...
    Private Sub btnadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnadd.Click
        Transaction_Form.txtSup_itemid.Text = dtglist.CurrentRow.Cells(0).Value
        Me.Close()
    End Sub
